...
function App() {
  // ── Stores ─────────────────────────────────────────────────────────────────
  const { tasks, customContexts, hiddenDefaultContexts, settingsLoaded, loadTasks, clearAllTasks } = useTaskStore();
  const {
  ...
  const contextOptions = Array.from(
    new Set([
      ...DEFAULT_CONTEXT_OPTIONS.filter((c) => !hiddenDefaultContexts.includes(c)),
      ...customContexts,
  ...
  async function handleExport() {
    try { await exportTasks(); } catch (err) { console.error("Export failed:", err); }
  }
  ...
  async function handleImportFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > 5 * 1024 * 1024) {
      const proceed = window.confirm(
        "This file is over 5MB, which is unusually large for a task export. Importing it may take a while and could cause performance issues.\n\nContinue anyway?"
      );
      if (!proceed) return;
    }
    ...
    try {
      ...
      setImportSuccess(`Imported ${imported.length} task${imported.length !== 1 ? "s" : ""}. Reloading…`);
    ...
    <div className="app">
      <header className="app-header">
        ...
        <div className="header-controls">
          <button type="button" className="faq-trigger" onClick={openFAQ} aria-label="Open FAQ">FAQ</button>

          <button type="button" className="settings-trigger" onClick={openSettings} aria-label="Open settings">
          ...
          <div className="advanced-toggle toolbar-row">
            <label className="toggle toggle--labeled">
              ...
              <input type="checkbox" checked={advancedFeaturesEnabled} onChange={(e) => setAdvancedFeaturesEnabled(e.target.checked)} aria-label="Advanced Features" />
      ...
      <main className="app-main">
        ...
        <section className="filters">
          <FilterBar contextOptions={contextOptions} snoozedCount={snoozedCount} />
        </section>

        <div className="mode-strip" role="group" aria-label="View modes">
          <button type="button" className="mode-pill" aria-pressed={viewMode === "custom" && !momentumModeEnabled}
            onClick={() => { setMomentumModeEnabled(false); setViewMode("custom"); }}>Custom</button>
          <button type="button" className="mode-pill" aria-pressed={viewMode === "sorted" && !momentumModeEnabled}
            onClick={() => { setMomentumModeEnabled(false); setViewMode("sorted"); }}>Sorted</button>
          {advancedFeaturesEnabled && (
            <button type="button" className="mode-pill" aria-pressed={momentumModeEnabled}
              onClick={() => setMomentumModeEnabled(true)}>Get Going</button>
          )}
        </div>
            ...
            <div className="mode-controls">
              ...
              {advancedFeaturesEnabled && momentumModeEnabled && (
                <div className="momentum-controls">
                  <MomentumPanel visibleTasks={visibleTasks} momentumNeedsFallback={!!momentumNeedsFallback} />
                </div>
              )}
            </div>
          </div>
        ...
        <section className="task-list-section">
          ...
          {showEmptyState && (
            <div className="empty-state">
              {noTasksAtAll && (<p>No tasks in list, <button type="button" className="empty-state__link" onClick={() => openTaskForm()}>add a task</button>.</p>)}
              {allDone && (<p>All done for today! ...or not quite? <button type="button" className="empty-state__link" onClick={() => openTaskForm()}>Add a new task.</button></p>)}
              {!noTasksAtAll && !allDone && (
                <>
                  {hasActiveFilters && (
                    <p>No visible tasks, {activeFilterCount} filter{activeFilterCount !== 1 ? "s" : ""} active.{" "}
                      <button type="button" className="empty-state__link" onClick={resetFilters}>Reset filters</button>
                    </p>
                  )}
                  {snoozedCount > 0 && (
                    <p>{snoozedCount} snoozed task{snoozedCount !== 1 ? "s" : ""}.{" "}
                      {filtersExpanded
                        ? <button type="button" className="empty-state__link" onClick={() => useTaskStore.getState().unsnoozeAll()}>Unsnooze</button>
                        : <button type="button" className="empty-state__link" onClick={() => setShowSnoozedTasks(true)}>View snoozed tasks</button>}
                    </p>
                  )}
                  {!hasActiveFilters && snoozedCount === 0 && (<p>No visible tasks right now.</p>)}
                </>
              )}
            </div>
          )}
          ...
          <ul className="task-list">
            {displayedTasks.map((task) => (
              <TaskCard key={task.id} task={task} contextOptions={contextOptions} visibleTasks={visibleTasks} isKeystone={task.id === keystoneTaskId} />
            ))}
          </ul>
